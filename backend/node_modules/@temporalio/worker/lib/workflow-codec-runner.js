...
class WorkflowCodecRunner {
    ...
    async encodeEventGroupMarkers(context, markers) {
        if (!markers?.length) {
            return undefined;
        }
        ...
        return await Promise.all(markers.map(async (marker) => ({
            ...
            label: marker.label
                ? {
                    ...marker.label,
                    label: await (0, internal_non_workflow_1.encodeOptionalSingle)(this.codecs, marker.label.label, context),
                }
                : undefined,
        })));
    }
                    ...
                    commands: completion.successful.commands
                        ? await Promise.all(completion.successful.commands.map(async (command) => {
                            ...
                            return {
                                ...
                                eventGroupMarkers: await this.encodeEventGroupMarkers(userMetadataContext, command.eventGroupMarkers),
                            };
                        }))
                        : null,
                }
